...
var detailview = $.extend({}, $.fn.datagrid.defaults.view, {
	render: function(target, container, frozen){
		...
		for(var i=0; i<rows.length; i++) {
			...
			table.push('<div class="datagrid-row-detail2"  >');
			if (frozen){
				table.push('&nbsp;');
			} else {
	...
	insertRow: function(target, index, row){
		...
		function _insert(frozen){
			...
			newDetail.hide();
			if (!frozen){
				newDetail.find('div.datagrid-row-detail2').html(opts.detailFormatter.call(target, index, row));
			}
		}
	},
	...
	onAfterRender: function(target){
		...
		// 	fr.hide();
		// 	var ww = ht.width()-1;
		// 	var details = dc.body2.find('>table.datagrid-btable>tbody>tr>td>div.datagrid-row-detail2:visible')._outerWidth(ww);
		// 	details.find('.easyui-fluid').trigger('_resize');
		// 	fr.show();
		// }
		function resizeDetails(){
			var details = dc.body2.find('>table.datagrid-btable>tbody>tr>td>div.datagrid-row-detail2:visible');
			if (details.length){
				var ww = 0;
				//2017-01-04 
				//注释的这行代码是因为点击其他选项卡后，第一个展开的detailview 的width直接修改为0；现将visible 属性删除
				//dc.header2.find('.datagrid-header-check:visible,.datagrid-cell:visible').each(function(){
				dc.header2.find('.datagrid-header-check,.datagrid-cell').each(function(){
		...
		var detail = dc.body1.add(dc.body2).find('div.datagrid-row-detail2');
...
$.extend($.fn.datagrid.methods, {
	...
	getRowDetail: function(jq, index){
		...
		// return tr.next().find('div.datagrid-row-detail2');
		
		return tr.next().find('>td>div.datagrid-row-detail2');
	},
...
$.extend($.fn.datagrid.methods, {
	subgrid: function(jq, conf){
		return jq.each(function(){
			...
			function setParentHeight(target){
				var tr = $(target).closest('div.datagrid-row-detail2').closest('tr').prev();
				if (tr.length){
	...
	getParentGrid: function(jq){
		var detail = jq.closest('div.datagrid-row-detail2');
		if (detail.length){
